...
describe('ReduxDataProxy', () => {
  ...
  describe('writeQuery', () => {
    it('will write some data to the store', () => {
      ...
      assert.deepEqual((proxy as any).store.getCache().getData(), {
        ROOT_QUERY: {
          a: 1,
        },
      });
...
describe('TransactionDataProxy', () => {
  ...
  describe('writeQuery', () => {
    ...
    it('will create writes that get returned when finished', () => {
      ...
      const document1 = addTypenameToDocument(
        gql`
          {
            a
            b
      ...
      const document2 = addTypenameToDocument(
        gql`
          {
            foo(id: $id) {
              d
